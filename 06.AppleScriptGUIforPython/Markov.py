...
def usage():
    print ("markov, version 1.0\n\nusage: ./Markov.py [-ht] [-o order] [-w paragraph wordlength] [-s start phrase] input_files\n")
    return
...
def create_markov_text_from(pathnames, markov_order=2, word_len=200, starting_words=''):
    ...
    for pathname in pathnames:
        ...
        if first_file == True:
            if timing:
                ...
                augment_result_string('\tElapsed time: ', round(now - zerotime, 2), 'seconds.\n')
...
def main(argv):
    ...
    for opt, value in opts:
        if opt == '-o':
            try:
                ...
                if MarkovOrder > 4:
                    high_markov_error = "\nWhat's the point? All this order will "
                    high_markov_error += "do is lift a block of verbatim text from "
                    high_markov_error += "one of your input files!\n"
    ...
    if timing:
        ...
        print ('\nThe creation of this Markov text took', round(stop - start, 2), 'seconds.\n')
    return
